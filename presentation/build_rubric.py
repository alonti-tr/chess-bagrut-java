...
from docx_helpers import (
    ...
    new_document, page_break, warn_box, word_postprocess_rtl, _set_bidi,
)
...
def write_part_zero(doc):
    ...
    add_table(doc, ["#", "דרישה", "סטטוס", "מיקום בפרויקט / הוכחה"], [
        ...
        ("2", "מערכת שרת/לקוח + תור לקוחות (ריבוי לקוחות במקביל = בונוס)",
         ...
         "במחלקת Matchmaker. תהליכון נפרד לכל לקוח (Thread) = ריבוי לקוחות במקביל"),
...
def write_part_a(doc):
    ...
    add_table(doc, ["מרכיב", "תיאור הדרישות", "משקל", "ניקוד", "סטטוס", "הערות"], [
        ...
        ("תוכן עניינים", "מקושר לפרקים", "2%", "___", "✅",
        ...
        ("עימוד אחיד", "שוליים ומרווחים", "3%", "___", "✅",
...
def write_part_b(doc):
    ...
    add_table(doc, ["תת-סעיף", "סטטוס", "מיקום / הערות"], [
        ("תיאור הרעיון והמוטיבציה", "✅", "תיק §1.1"),
        ("ייזום: זיהוי הצורך", "✅", "תיק §1.2"),
        ("אפיון פונקציונלי", "✅", "תיק §1.3 — 10 דרישות"),
        ("אפיון לא-פונקציונלי", "✅", "תיק §1.4 — 5 דרישות איכות"),
        ("לוח זמנים הגיוני", "✅", "תיק §1.5 — אבני דרך"),
        ("ניהול סיכונים", "✅", "תיק §1.6 — 4 סיכונים עיקריים"),
    ])
    he_paragraph(doc, "ניקוד מבוא: ___ / 13%", bold=True, color=ACCENT)

    heading(doc, "2. תיאור תחום הידע — פרק מילולי (ניתוח) — 10%", level=2)
    add_table(doc, ["תת-סעיף", "סטטוס", "מיקום / הערות"], [
        ("חוקי שחמט וניהול משחק", "✅", "תיק §2.1 — כולל קידום, en passant והצרחה"),
        ("תקשורת רשת — שכבות ו-TCP", "✅", "תיק §2.2"),
        ("גיבוב סיסמאות (SHA-256 + salt)", "✅", "תיק §2.3"),
        ("ריבוי תהליכונים", "✅", "תיק §2.4 — Thread + synchronized"),
        ("שמירת נתונים בקובץ", "✅", "תיק §2.5"),
    ])
    he_paragraph(doc, "ניקוד ניתוח: ___ / 10%", bold=True, color=ACCENT)

    heading(doc, "3. מבנה / ארכיטקטורה (העיצוב) — 25%", level=2)
    add_table(doc, ["תת-סעיף", "סטטוס", "מיקום / הערות"], [
        ("שרטוט ארכיטקטורה", "✅", "תיק §3.1 — תרשים בלוקים"),
        ("תרשימי זרימת מידע", "✅", "תיק §3.2 — 4 דיאגרמות (כולל קידום, הצרחה, הדגשת מהלכים)"),
        ("הבחנה בין מודולי שרת ולקוח", "✅", "תיק §3.3 — טבלת חבילות"),
        ("ניתוח אלגוריתמים + שקילת חלופות", "✅", "תיק §3.4 — חוקיות מהלך, AI, פרוטוקול"),
        ("פרוטוקול תקשורת הגיוני", "✅", "תיק §3.5 — מכונת מצבים"),
    ])
    ...
    add_table(doc, ["תת-סעיף", "סטטוס", "מיקום / הערות"], [
        ("הסבר התקנה והרצה", "✅", "תיק §5.1 — JDK 17 + run.bat"),
        ("צילומי מסך של הזרימה", "⚠️", "תיק §5.2 — מקום מסומן ל-7 צילומים שעל התלמיד להוסיף (כולל הצרחה ונקודות ירוקות)"),
        ("הסברים לכל סוגי המשתמשים", "✅", "תיק §5.3 — אדם מול אדם, מול מחשב"),
        ("הוכחת הצפנת הסיסמאות", "✅", "תיק §5.4 — תוכן users.json"),
    ])
    ...
    add_table(doc, ["תת-סעיף", "סטטוס", "מיקום / הערות"], [
        ("מה למדתי על עצמי", "⚠️", "תיק §6.1 — תבנית; חייב מילוי אישי ע\"י התלמיד"),
        ("מה למדתי מקצועית", "✅", "תיק §6.2 — לקחים מקצועיים"),
        ("מה הייתי משנה", "✅", "תיק §6.3 — שיפורים עתידיים"),
    ])
    he_paragraph(doc, "ניקוד רפלקציה: ___ / 6%", bold=True, color=ACCENT)

    heading(doc, "7. ביבליוגרפיה — 5%", level=2)
    add_table(doc, ["תת-סעיף", "סטטוס", "מיקום / הערות"], [
        ("סקר ספרות (לא רק רשימת קישורים)", "✅",
    ...
    add_table(doc, ["תת-סעיף", "סטטוס", "מיקום / הערות"], [
        ("תדפיס הקוד", "✅", "נספח א' — תדפיס כל קבצי הקוד (19 קבצים)"),
        ("טבלת הודעות הפרוטוקול", "✅", "נספח ב' — הודעות + דוגמת זרימה"),
        ("שאלות תיאורטיות לבחינה", "✅", "נספח ג' — תקשורת, גיבוב, OS, OOP"),
    ])

    heading(doc, "בונוס — עד 10%", level=2)
    add_table(doc, ["מרכיב בונוס", "סטטוס בפרויקט", "ניקוד מומלץ"], [
        ("ריבוי לקוחות במקביל", "✅ תהליכון לכל לקוח (Thread)", "___"),
        ("בינה מלאכותית", "✅ ChessAI חמדנית", "___"),
        ("היקף וקריאות הקוד", "✅ מבנה מסודר ב-OOP עם חבילות; הצרחה + הדגשת מהלכים", "___"),
    ])
...
def write_oral_rubric(doc):
    ...
    add_table(doc, ["מרכיב", "משקל", "ניקוד", "קריטריונים", "הפניות בפרויקט"], [
        ...
        ("שליטה בקוד", "30%", "___",
         ...
         "users.json + UserAuth (load/save) + גיבוב SHA-256"),
        ("שליטה בחומר התיאורטי", "20%", "___",
         "תקשורת, גיבוב, מערכת הפעלה, OOP",
...
def write_examiner_notes(doc):
    ...
    he_bullet(doc, "פרק 2 — ניתוח: חוקי שחמט, תקשורת ו-TCP, גיבוב סיסמאות, תהליכונים, קבצים")
