...
def write_intro(doc):
    ...
    he_paragraph(doc,
                 ...
                 "פונקציית ערבול (hash) מסוג SHA-256 עם salt אקראי לכל משתמש.")
    he_paragraph(doc,
                 ...
                 "פשוט, שמירת נתונים בקובץ, ערבול סיסמאות, וממשק משתמש גרפי ב-Swing.")
    ...
    add_table(doc, ["#", "תכונה", "יעד"], [
        ("NFR-1", "אבטחת סיסמאות", "סיסמאות נשמרות מעורבלות בלבד (SHA-256 + salt). אין סיסמה גלויה בקובץ users.json"),
        ("NFR-2", "יציבות שרת", "ניתוק של לקוח אחד אינו מפיל את השרת או משחקים אחרים"),
        ("NFR-3", "ריבוי משחקים", "השרת תומך בכמה משחקים במקביל בעזרת נימה לכל לקוח"),
    ...
    add_table(doc, ["סיכון", "הסתברות", "חומרה", "מיטיגציה"], [
        ...
        ("שני תהליכונים ניגשים לאותו משחק", "בינונית", "בינונית",
         "מתודות המשחק מסומנות synchronized — רק נימה אחת מבצעת מהלך בכל רגע"),
...
def write_knowledge_domain(doc):
    ...
    heading(doc, "2.3 ערבול סיסמאות (Hashing)", level=2)
    he_paragraph(doc,
                 ...
                 "פונקציית ערבול סטנדרטית שמחזירה פלט באורך קבוע (256 ביט), דרך "
    ...
    callout(doc, [
        ("הבחנה חשובה: ", "bold"),
        ("ערבול (hashing) הוא לא הצפנה. הצפנה היא דו-כיוונית (אפשר לפענח בחזרה), "
         "ואילו ערבול הוא חד-כיווני. לסיסמאות משתמשים בערבול, כי השרת לא צריך לדעת "
         "את הסיסמה המקורית — רק לבדוק אם הסיסמה שהוזנה נותנת את אותו hash.", "he"),
    ])

    heading(doc, "2.4 ריבוי נימות (Threads)", level=2)
    he_paragraph(doc, "ההבדל בין תהליך (Process) לנימה (Thread):")
    he_bullet(doc, "תהליך (Process): תוכנית רצה עם זיכרון נפרד משלה.")
    he_bullet(doc, "נימה (Thread): יחידת ריצה בתוך אותה תוכנית; כל הנימות חולקות את אותו זיכרון.")
    he_paragraph(doc,
                 "בשרת אני משתמש בנימה נפרדת לכל לקוח שמתחבר. כך השרת יכול לטפל "
                 "בכמה לקוחות ובכמה משחקים בו-זמנית, בלי שלקוח אחד יחסום את האחרים.")
    he_paragraph(doc,
                 "כשכמה נימות ניגשות לאותו מידע משותף (למשל אותו משחק), צריך "
                 "להגן עליו. ב-Java עשיתי זאת באמצעות המילה synchronized על מתודות "
                 "המשחק והתור — כך רק נימה אחת נכנסת בכל פעם, והמידע לא מתקלקל.")
    code_block(doc, [
        "// ChessServer.java - נימה לכל לקוח",
...
def write_architecture(doc):
    ...
    add_table(doc, ["קובץ / חבילה", "צד", "תפקיד"], [
        ...
        ("chess.UserAuth", "שרת", "ערבול סיסמאות ושמירה בקובץ"),
...
def write_implementation(doc):
    ...
    add_table(doc, ["מחלקה", "חבילה", "תפקיד"], [
        ...
        ("UserAuth", "chess", "הרשמה/התחברות + ערבול סיסמאות + users.json"),
...
def write_reflection(doc):
    ...
    he_bullet(doc, "ההבדל בין ערבול (hash, חד-כיווני, לסיסמאות) לבין הצפנה (דו-כיוונית).")
    ...
    he_bullet(doc, "למה צריך נימה לכל לקוח, ולמה צריך synchronized על מידע משותף.")
...
def write_bibliography(doc):
    ...
    sources = [
        ...
        ("[2] תיעוד Java Concurrency (Thread, synchronized)",
         ...
         "עזר להבין Thread ו-synchronized, ואיך מריצים נימה נפרדת לכל לקוח."),
...
def write_qa_appendix(doc):
    ...
    heading(doc, "ג.2 ערבול והצפנה", level=2)
    qa = [
        ("מה זה hash?",
    ...
    qa = [
        ...
        ("מה זה synchronized ולמה צריך אותו?",
         "מילת מפתח שמבטיחה שרק נימה אחת תריץ את המתודה בו-זמנית, כדי שמידע "
